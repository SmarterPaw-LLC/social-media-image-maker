...
--    file_size_limit = NULL: no per-bucket cap. The Supabase plan ceiling
--    (single-file 50 GB on Pro) is the only ceiling. Earlier revisions of
--    this script capped at 200 MB which blocked larger image-heavy design
--    states; lifted because the project is on a paid plan and the per-bucket
--    cap was redundant. The proper long-term fix is still to move image
--    data URIs out of state.json into separate Storage files referenced by
--    URL, so design state stays under a few KB.
-- ──────────────────────────────────────────────────────────────────────────
insert into storage.buckets (id, name, public, file_size_limit)
  values ('design-states', 'design-states', false, null)
